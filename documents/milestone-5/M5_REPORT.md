...
As the conclusions presenting in the [Milestone 3 report][M3] demonstrate, the current limitations of UPLC are such that implementing a parametric onchain zero-knowledge proof verifier is not viable, specifically due to the intractable performance problems that emerge when attempting to implement operations on higher-order field extensions and curves over those extensions. 

Due to these limitations, we were forced to halt our attempt to develop this verification framework. These limitations, however, are not essential features of UPLC, and the language could be modified in such a way as to make our original goal viable. 

In this report, we will outline three proposed changes to UPLC: Call by need evaluation, mutability, and a better implementation of arrays.  Each of our proposed changes would, on its own, likely suffice to solve the problems we encountered, and we will explain why we believe that to be the case. We will also discuss the tradeoffs of each proposed change, with particular attention to the amount of to the existing UPLC codebase. 

## Call-by-Need (Laziness)

FOR ALL THESE SECTIONS DO: 
  1. EXPLAIN THE CHANGE CONCEPTUALLY
  2. EXPLAIN WHY IT WILL SOLVE OUR PROBLEM 
  3. EXPLAIN THE IMPACT ON UPLC

Perhaps the most obvious and parsimonious solution to the problems we encountered is modifying UPLC to support call by need (also commonly known as "lazy") evaluation. 
...
In Haskell, the expression `10^10` is only computed _once_ and, supposed that GHC works as it should, is cached after evaluation. We can attempt to mimic this in UPLC (which is presented here with Haskell-like pseudocode due to the lack of a standard "readable" UPLC representation): 

```haskell
(\x -> force x + force x) (delay (10^10))
```

Doing so will defer the evaluation of `10^10` until it is forced, but because no caching or memoization occurs, the expression `10^10` must be evaluated _twice_ here. 
...
The second option for modifying UPLC so that it can facilitate performant onchain verification is to add _mutability_ to the language.

For our purposes, "local" mutability would suffice. By this we mean mutability that is essentially analogous to Haskell's `ST`, where variables must be explicitly tagged as mutable and cannot escape their computational context. (Rust handles mutation in a somewhat similar way, though the details differ). 
...
Moreover, we believe it is very likely that global mutable state would have severe and significant implications for the metatheoretical basis of the Plutus language, quite possibly in ways that make it difficult or impossible to prove certain important properties about the language. 

## Better Arrays 

The third option for modifying UPLC to support parametric onchain verification is to _improve the existing `Array` API_. 

At present, the default UPLC universe supports `Array`s (as a wrapper over strict `Data.Vector.Strict.Vector`s). UPLC also supports several basic operations provided as builtins: `LengthOfArray`, `ListToArray`, and `IndexArray`, which function as the names would indicate. 
...
The problem, for our purposes, with the existing `Array` API is not that it is defective, but rather that it is deficient: It lacks several essential operations which every other implementation of arrays (that we are familiar with) does not lack. In particular, the existing UPLC array API does not provide: 
  1. A way to _construct arrays as runtime constants_. The existing API requires first constructing a list, and then converting to an array, which introduces nontrivial O(n) overhead.
...
Nonetheless, we believe that an improved `Array` API which supports _limited_ mutatability is likely the best option here. This solution is what we propose in the [MLabs Better Arrays proposal][BETTER_ARRAYS]. Our proposal, however, requires some explanation in the context of this report, because it is not a straightforward implementation of mutable `Array`s that readers might naively expect. Technically speaking, our proposal is a plan to implement [defunctionalized push arrays][DEFUN-PUSH-ARRAYS]. Before explaining what our proposal is, and how it can give us restricted mutability without the issues that come with global mutable variables, we first have to explain the difference between *pull arrays* and *push arrays*.
...
It is important to be explicit that these additions to UPLC are not, on their own, an implementation of defunctionalized push arrays. As we state in our proposal, these builtins are intended for "internal" use by compilers of UPLC frontend languages. Should that proposal be funded, part of our work will be to modify the compilers for existing frontend languages, so that frontend languages only expose a safe interface which is then transformed into calls to these functions in a code transformation or code generation pass. These additions to UPLC, however, suffice for the implementation of defunctionalized push arrays in a frontend language without directly exposing unsafe operations to users. 

## NOTES (delete later)

NOTES: 

Whenever we deal w/ a packed memory structure in a lang where mutability is not implicit, we can't have everything we want. Namely: 
  - If we care about immutability, we have to either use some kind of functional approximation (which is not going to be packed in memory b/c functional data sturcutres use pointers for sharing)
  - or we have to severely limit what we can do to ensure this is maintained 
  
Implicit mutability w/ true arrays is where arrays exist as a structure that can be mutated anywhere. Option 1. 

In UPLC arrays cannot be mutated at all (option 2). 

CANNOT JUST LIFT VECTOR FUNCTIONS BECAUSE CANNOT COST HIGHER ORDER FUNCTIONS, SEE https://github.com/cardano-foundation/CIPs/blob/master/CPS-0029/README.md

Three options for having something that behaves like an array and allows some notion of mutation w/o underlying raw mutation primitives: 
  1. Pull Arrays: Basically a function that computes the value at any index for its length. This can behave like an array for a large number of operations. Can map/fold/slice. Lets you avoid intermediaries. There are several shortcomings though: 
       a. Can't index efficiently 
       b. Really bad for concatenation 
       c. Really bad for any non-affine transformation (anything that adds or removes elements) 
       d. Really bad for anything that changes the array's structure 
       e. Single position modifications are also really bad 
       So: This limits severely what you can do w/ pull arrays and plutarch pull arrays won't really help 
       aside: Vector is basically a cached evaluation of a pull array 
  2. Push Arrays: Can be thought of as a DSL describing how an array would be assembled.
    - This solves a lot of problems: Concat/etc are really efficient 
    - `data Push a = Push Int ((Int -> CM ()) -> CM ())`
    - CM requires low level array primitives like: Create an array, write to position i of array with y, etc 
    - Big problem: No builtins suffice for an implementation of CM so we can't do this
      - BUT: BETTER ARRAYS IS AN IMPLEMENTATION OF CM. `CreateArrayBuilder`/`WriteArrayBuilder`/`ArrayBuilderToArray` suffice for CM
    - Second problem: Can't really do this at runtime. 
    - Solution: Defunctionalized push arrays. Instead of building out the CM at runtime, we do it at compile time, and generate the code we want. See https://dl.acm.org/doi/epdf/10.1145/2636228.2636231
  3. Staging: You completely erase everything and do everything at compile time. Can think of as one more step of defunctionalization, array primitives get put in as compiler primitives, compiler does the work when we generate code. More like a compiler plugin than a DSL. 
  
OK, even if we have all of this, how does it help us given that we are using affine reprs? 

Reason: This gives us a very limited form of mutation without global mutability. Something that looks like mutable arrays should be tolerable. Because we could represent pairs (or larger tuples) as arrays, it follows that we could (potentially) get the benefits of mutability w/o having to affect everything. 

Under the hood, Vector really does treat immutable vectors (largely) as pull arrays. If you only ever work in the immutable vector, it's like a hybrid of pull arrays and streams w/ the ability to materialize intermediate arrays when required. 

There's also the mutable interface which lets you use mutation directly w/ a monadic "firewall" between immutable and mutable worlds. 

Massiv has those two, plus push arrays, plus parallel evaluation, etc. 
